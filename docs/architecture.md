...
## 7.0 根部（包级入口、宿主组合与横切契约）

Host 根不属于任何业务模块，只承担三类职责：

* **包级入口**：插件入口与模式二服务进程入口（后者是包级二级入口，发布路径固定）。入口只做装配与协调——解析启动参数、装配模块能力、协调进程/插件退出，不承载业务语义；
* **宿主组合**：装配各模块公共能力、观察官方事件（翻译与转发）、汇总生命周期与清理。每子代理作用域装配的安装/撤销归宿主层（见 §7.5）；
* **宿主级横切契约**：配置契约、官方事件的本地声明、跨模块共用的最小结构契约与纯解析规则（多模块以相同语义消费、且不拥有业务状态的能力）。

边界（跨模块规则见 `src/host/AGENTS.md`）：

* **单一来源**：同一官方能力的最小结构契约（webServer 注册契约）与同一官方服务的枚举投影（agentPresets / llm）全 Host 只允许一处实现，各模块消费而非复制；
* **入口薄**：装配默认值只来自配置契约（schema 为唯一来源），入口不得复制默认值清单、校验清单或字段映射；
* **零状态横切契约**：只含纯结构类型与纯函数（解析、投影、校验），不持有实例状态、缓存、定时器，也不替调用方决定降级语义；
* **只收窄不伪造**：官方事件与 payload 的本地声明只声明本插件真正消费的字段，形状漂移一律运行时守卫 + 降级。

文件职责：

```text
index.ts                 插件入口（apply + 公共 re-export；发布契约路径）
service-runner.ts        模式二服务进程入口（包级二级入口；只做装配与协调）
...
* **契约单一来源**：端点名取自共享协议常量，白名单与其零漂移；webServer 注册契约取自宿主级横切契约（host 根），边界内不自建第二份最小结构；
...
* **入口只做装配**：服务进程入口（包级二级入口）只解析启动参数、装配模块能力与协调退出，进程内实现（会话根 Agent 取/建、协议核心）归本模块；webServer 注册契约取宿主级横切契约，不自建第二份。
