...
//
// tests/client/components/canvas/GraphCanvas.test.tsx
//
...
describe('GraphCanvas 连线条件着色（条件优先于通道）', () => {
  it('db 连线设 pass 条件 → 渲染 class 含 is-pass（而非 is-db）', () => {
    const a: CanvasNode = { id: 'a', kind: 'agent', position: { x: 0, y: 0 }, data: { label: 'A' } }
    const b: CanvasNode = { id: 'b', kind: 'database', position: { x: 320, y: 0 }, data: { label: 'B' } }
    const dbEdge: CanvasEdge = {
      id: 'e-db',
      source: 'a',
      target: 'b',
      sourceHandle: 'db-out',
    ...
    const edge = container!.querySelector('path.wf-graph__edge')
    expect(edge).not.toBeNull()
    expect(edge!.classList.contains('is-pass')).toBe(true)
    expect(edge!.classList.contains('is-db')).toBe(false)
  })
})
