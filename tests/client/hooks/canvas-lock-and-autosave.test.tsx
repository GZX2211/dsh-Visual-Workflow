...
// tests/client/hooks/canvas-lock-and-autosave.test.tsx
//
// 新机制单测（用户裁决 2026-02）：
...
describe('运行中保存实例的二次确认（需求 b）', () => {
  ...
  it('保存时显式传入新坐标 → 落库载荷用新坐标（不是闭包里的旧画布）', async () => {
    const { state } = runningState()
    const dispatch = vi.fn()
    const saveWorkflow = vi.fn(async (flow: WorkflowDocument): Promise<WorkflowDocument | null> => ({ ...flow, revision: 2 }))
    const face = await renderDocument({ state, dispatch, saveWorkflow })

    // 自动布局/拖动结束这类「同批 dispatch 后立即落盘」的调用方显式传 nodes
    const nextNodes = [node('n-next'), { ...node('n-next'), position: { x: 120, y: 64 } }]
    await act(async () => {
      await face.saveCanvas({ auto: true, nodes: nextNodes, onSaved: () => {} })
    })

    expect(saveWorkflow).toHaveBeenCalledTimes(1)
    const saveArgs = (saveWorkflow.mock.calls as unknown as Array<[WorkflowDocument, CanvasNode[], CanvasEdge[]]>)[0]!
    expect(saveArgs[1]).toBe(nextNodes)
    expect(saveArgs[2]).toBe(state.canvas.edges)
  })

  it('运行中「保存并继续」：确认后真实落库成功才触发 onSaved', async () => {
    const { state } = runningState()
    const dispatch = vi.fn()
    const saveWorkflow = vi.fn(async (flow: WorkflowDocument): Promise<WorkflowDocument | null> => ({ ...flow, revision: 2 }))
    const onSaved = vi.fn()
    ...
    await act(async () => {
      saved = await face.saveCanvas({ onSaved })
    })
    // 需二次确认：本次调用没落库 → 返回 null、onSaved 尚未触发
  ...
  it('运行中「保存并继续」：确认后落库失败 → 不触发 onSaved（原操作不继续）', async () => {
    const { state } = runningState()
    const dispatch = vi.fn()
    const saveWorkflow = vi.fn(async () => { throw new Error('put failed') })
    const onSaved = vi.fn()
    const face = await renderDocument({ state, dispatch, saveWorkflow })

    await act(async () => {
      await face.saveCanvas({ onSaved })
    })
    const confirm = dispatch.mock.calls
      .map(([action]) => action as { type: string; confirm?: { onConfirm?: () => void } })
      .find((action) => action.type === 'CONFIRM_SET' && action.confirm)?.confirm
    ...
    expect(saveWorkflow).toHaveBeenCalledTimes(1)
    expect(onSaved).not.toHaveBeenCalled()
    expect(dispatch.mock.calls.some(([action]) => (action as { type: string }).type === 'MARK_SAVED')).toBe(false)
  })
})
...
describe('取消二次确认：模板删除直接执行（用户裁决 2026-02）', () => {
  ...
  it('删除工作流模板：不再弹确认，直接调用后端删除并清空画布', async () => {
    // 真实形态：被删模板就是当前打开的文档（currentId + currentKind 齐备），
    // 归属校验因此判定「仍是当前文档」→ 照旧清空画布。
    const state: StudioState = {
      ...createInitialState('s-1'),
      currentId: 't-1',
    ...
    const clearCount = dispatch.mock.calls.filter(([action]) => (action as { type: string }).type === 'CLEAR_CANVAS').length
