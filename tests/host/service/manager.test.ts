...
//   崩溃标记 crashed、stop（SIGTERM/宽限期强杀定时器）、autoRecover（仅重启 running +
//   端口重分配）、resolveDshCommand（PATH 解析/缺失明确错误）。
...
import { ServiceManager, resolveDshCommand, SERVICE_ERR, STOP_GRACE_MS } from '../../../src/host/service/index.js'
...
describe('ServiceManager 生命周期', () => {
  ...
  it('重复 stop：撤销上一次的强杀定时器（宽限期不双份计时）', async () => {
    ...
    const h = makeManager(store, store.root)
    await h.manager.start('svc-1')
    // _destroyed 是 Node Timeout 对象的销毁标记（clearTimeout 后为 true）：
    // 用它观测「旧定时器是否已撤销」，避免真实等待 5s 或真杀进程。
    const internals = h.manager as unknown as { children: Map<string, { forceKill: { _destroyed?: boolean } | null }> }
    await h.manager.stop('svc-1')
    const first = internals.children.get('svc-1')?.forceKill
    expect(first?._destroyed).toBe(false)
    await expect(h.manager.stop('svc-1')).resolves.toMatchObject({ serviceId: 'svc-1', status: 'stopped' })
    const second = internals.children.get('svc-1')?.forceKill
    expect(first?._destroyed).toBe(true)
    expect(second).not.toBe(first)
    // 收尾：exit 清理内存条目与其强杀定时器（避免测试结束后 5s 触发强杀）
    h.children[0].emitExit(0)
  })
