// tests/host/service/sessions-map.test.ts
//
// userId→sessionId 映射单测（T-033）：映射隔离（不同 userId 独立）/同 userId 稳定/
// 磁盘持久化/服务重启恢复/并发解析单 sessionId/映射文件按服务隔离。
//
// 归属说明：断言对象是 SessionMap 自身的稳定契约（FlowStore 是它的真实依赖，不是被验证
// 对象）——与 tests/host 下其它「单模块 + 真实 FlowStore」的单测同层，故不再放 integration。
...
import { SessionMap, SESSION_ID_PREFIX } from '../../../src/host/service/index.js'
