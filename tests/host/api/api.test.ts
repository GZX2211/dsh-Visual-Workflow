...
//   - 导入导出：v2 bundle 冲突/rename/overwrite、角色模板往返（领域错误码）；
//   - MCP 端点：托管区读写（DSH_HOME 指向临时目录）；
//   - 路由注册：disposer、非 POST 405、无效 JSON 400、服务端点无管理器 501、
//     领域错误码 → HTTP 状态映射、受管文件下载（GET + 目录穿越拒绝）。
...
import { VisualWorkflowApi, registerRoutes, type ApiHost } from '../../../src/host/api/index.js'
import { CARD_DESC_MAX, zhDescription } from '../../../src/host/api/tool-descriptions.js'
import { registerDownloadRoute } from '../../../src/host/api/file-download.js'
...
describe('导入导出', () => {
  ...
  it('角色模板导出/导入往返；非法文件 400/422', async () => {
    ...
    // 导入导出领域错误：领域模块抛稳定领域码（HTTP 状态由边界映射，见路由注册用例）
    await expect(h.api.handle('importWorkflow', { sessionId: 'session-1', json: 'not-json' })).rejects.toMatchObject({
