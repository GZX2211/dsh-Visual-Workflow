...
import { localToUtc, SchedulerEngine, SchedulerTaskStore, type SchedulerOrchestrator } from '../../../src/host/scheduler/index.js'
...
describe('调度引擎', () => {
  ...
  it('重入去重：重叠 tick 共享同一次扫描（同一任务不被并发决策）', async () => {
    const h = await makeHarness(makeTask())
    const listSpy = vi.spyOn(h.taskStore, 'list')
    // 两次 tick 在同一同步块内到达：第一次扫描在途，第二次不得再扫一遍
    const first = h.engine.sweep()
    const second = h.engine.sweep()
  ...
  it('start 幂等：只有一个定时器；disposer 调用后定时器清空', async () => {
    const h = await makeHarness(makeTask())
    const timerOf = (): unknown => (h.engine as unknown as { timer: unknown }).timer
    const disposer = h.engine.start()
  ...
  it('dispose 后 start 不再启动定时器（终止后不复活），sweep 也不再决策', async () => {
    const h = await makeHarness(makeTask())
    h.engine.dispose()
    const disposer = h.engine.start()
    expect((h.engine as unknown as { timer: unknown }).timer).toBeNull()
