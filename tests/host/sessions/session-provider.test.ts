// tests/host/sessions/session-provider.test.ts
//
...
//
// resolveNewSessionCwd（新会话工作目录决策的唯一实现）：
//   - 显式 workspacePath 优先（运行期不校验存在性，校验属保存端点）；
//   - 否则继承创建者会话 cwd（解析失败降级为省略）。

import { describe, expect, it, vi } from 'vitest'
import { CordisSessionProvider, resolveNewSessionCwd, sessionCwdResolver } from '../../../src/host/sessions/session-provider.js'
...
describe('resolveNewSessionCwd（新会话工作目录决策的唯一实现）', () => {
  it('显式 workspacePath 优先：不读创建者会话 cwd', async () => {
    const sessionCwdOf = vi.fn(async () => 'D:\\creator')
    await expect(resolveNewSessionCwd({
      workspacePath: 'D:\\explicit',
      creatorSessionId: 's-1',
      sessionCwdOf,
    })).resolves.toBe('D:\\explicit')
    expect(sessionCwdOf).not.toHaveBeenCalled()
  })

  it('显式路径裁剪两侧空白；纯空白视为未配置（走继承）', async () => {
    await expect(resolveNewSessionCwd({ workspacePath: '  D:\\explicit  ' })).resolves.toBe('D:\\explicit')
    await expect(resolveNewSessionCwd({
      workspacePath: '   ',
      creatorSessionId: 's-1',
      sessionCwdOf: async () => 'D:\\creator',
    })).resolves.toBe('D:\\creator')
  })

  it('无显式路径时继承创建者 cwd；缺创建者会话或缺解析能力 → undefined（省略 cwd 字段）', async () => {
    await expect(resolveNewSessionCwd({
      creatorSessionId: 's-1',
      sessionCwdOf: async () => 'D:\\creator',
    })).resolves.toBe('D:\\creator')
    await expect(resolveNewSessionCwd({ creatorSessionId: 's-1' })).resolves.toBeUndefined()
    await expect(resolveNewSessionCwd({ sessionCwdOf: async () => 'D:\\creator' })).resolves.toBeUndefined()
    await expect(resolveNewSessionCwd({})).resolves.toBeUndefined()
  })

  it('继承解析失败：降级为 undefined（不阻断新会话创建）', async () => {
    await expect(resolveNewSessionCwd({
      creatorSessionId: 's-1',
      sessionCwdOf: async () => { throw new Error('boom') },
    })).resolves.toBeUndefined()
  })

  it('只做选择不做校验：不存在的显式路径原样返回（存在性校验属保存端点）', async () => {
    await expect(resolveNewSessionCwd({ workspacePath: 'Z:\\not-exists' })).resolves.toBe('Z:\\not-exists')
  })
})
