window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		function sweep(targetLayer, referenceIndexOf, neighboursOf) {
			...
			const decorated = targetLayer.map((id) => {
				return {
					id,
					center: barycenter(id, neighboursOf(id), referenceIndexOf) ?? currentIndex.get(id) ?? 0,
		...
		*/
		...
		function layoutUnitsOf(inputs, collapse) {
		...
		*/
		function layoutGraph(nodes, lines, options = {}) {
			...
			const participating = /* @__PURE__ */ new Set();
			...
			const forwardEdges = [];
			const forwardLineIds = /* @__PURE__ */ new Set();
			...
			const rawLayers = Array.from({ length: Math.max(1, ...[...layerMembers.keys()].map((layer) => layer + 1)) }, () => []);
			...
			const orderedLayers = orderLayers({
				layers: rawLayers,
				adjacency,
				reverse,
				rounds
			});
			const layerOfUnit = /* @__PURE__ */ new Map();
			...
			const columnHeights = orderedLayers.map((layer) => layer.reduce((sum, id) => sum + heightOf(id), 0) + Math.max(0, layer.length - 1) * gutterY);
			...
			const placeInColumn = (layer, id) => {
				...
				return {
					x: columnX[layer] ?? originX,
					y: originY + offset + before
				};
			};
			for (const [id, layer] of layerOfUnit) {
				...
				if (needed > (Number(unit.height) || 0)) warnings.push(`协作组「${unit.id}」卡片高度不足，建议加高到 ${Math.round(needed)}px 以容纳 ${memberIds.length} 名成员`);
			}
			if (isolatedUnits.length > 0) {
				const orphanX = columnX.length > 0 ? (columnX[columnX.length - 1] ?? originX) + (columnWidths[columnWidths.length - 1] ?? 0) + gutterX : originX;
				let cursorY = originY;
			...
			for (const node of inputs) {
				...
				if (mainPosition) {
					...
					if (mainCol !== void 0) colOf.set(node.id, mainCol);
					continue;
				}
				const fallbackLayer = Math.max(0, orderedLayers.length - 1);
				positions.set(node.id, {
					x: columnX[fallbackLayer] ?? originX,
					y: originY
				});
		...
		*/
		function layoutBoxesOf(nodes, sizeOf, excludedIds = []) {
			...
			return (nodes ?? []).filter((node) => node.kind !== "proxy" && !members.has(node.id)).map((node) => {
				const size = sizeOf(node);
				return {
					id: node.id,
		...
		*/
		function collapsedIdsOf(inputs) {
			...
			for (const input of inputs ?? []) if (input.kind === "proxy") collapsed.add(input.id);
		...
		*/
		...
		function useEditorActions(state, dispatch, notify, toastError, t, workflows, flowTemplates, templates, selection, remote, saveCanvas, removeSelected, removeLine, selectWorkflow, selectFlowTemplate, options) {
			const { locks } = options;
			return {
		...
		*/
		...
		function GroupCard({ node, copy, members, selected, dropTarget, locked, lockHint, onPointerDown, onHandlePointerDown, onMemberSelect, onResizeStart }) {
			const size = nodeSizeOf(node);
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				onPointerDown: (event) => onPointerDown(event, node.id),
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: `wf-node wf-node--group${selected ? " is-selected" : ""}${dropTarget ? " is-drop-target" : ""}${locked ? " is-locked" : ""}`,
					title: locked ? lockHint : void 0,
					children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: "wf-node__kind",
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: String(copy.nodeKinds?.group ?? "协作组") }),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									className: "wf-hint",
		...
		*/
		function useAutoLayout(state, dispatch, options = {}) {
			...
			(0, react.useEffect)(() => {
				...
				if (needsAutoLayout(nodes)) {
					const next = applyLayout(nodes, layoutGraph(toLayoutInputs(nodes, (node) => groupCardSizeOf(node)), canvas.edges));
					dispatch({
						type: "GRAPH_REPLACED",
						nodes: next,
						edges: canvas.edges,
					...
					Promise.resolve(optionsRef.current.saveCanvas?.({ auto: true })).catch(() => {});
					optionsRef.current.onApplied?.(next);
					return;
				}
				if (findLayoutOverlaps(layoutBoxesOf(nodes, (node) => groupCardSizeOf(node), collapsedIdsOf(toLayoutInputs(nodes, (node) => groupCardSizeOf(node))))).length === 0) return;
		...
		function StudioLayout(props) {
			const { t, state, sessionId, remote, currentFlow, currentService, currentFlowTemplate, editorData, edgeList, stageKinds, parentTemplate, roleTemplates, groupTemplates, toolbarRunning, runStatusByNode, highlightedNodeIds, modeName, lockedNodeIds, lockedEdgeIds, instanceRunning, canvasApiRef, canvasShellRef, libraryImportRef, personaInputRef, groupMdInputRef, dispatch, doc, canvas, editor, run, transfer, selection, history, guard, panels, toast, beginLibraryDrag, dragPreview, dropGroupId, modeMenuOpen, setModeMenuOpen, switchMode, canvasCaption, leftOpen, bottomOpen, inspectorOpen, handleRun, panelsCollapsed, onTogglePanels } = props;
...
.wf-combo-card{display:flex;gap:9px;align-items:flex-start;text-align:left;border:1px solid var(--wf-border);border-radius:11px;background:var(--wf-layer-2);padding:10px 11px;cursor:pointer;transition:border-color .14s ease,background .14s ease;min-width:0;overflow:hidden}
...
.wf-combo-card__body{min-width:0;display:flex;flex-direction:column;gap:3px;overflow:hidden}
.wf-combo-card__name{font-size:12px;font-weight:650;color:var(--wf-ink);word-break:break-all}
.wf-combo-card__desc{font-size:10px;line-height:1.45;color:var(--wf-ink-2);display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden;overflow-wrap:anywhere;word-break:break-word;max-height:calc(1.45em * 3)}
