	...
	factory: (require) => {
		...
		*/
		...
		function Studio({ t, sessionId, remote: remoteProp, onRunImmersive }) {
			...
			const canvasCaption = currentFlowTemplate ? `${t.canvasCaptionTemplate}${currentFlowTemplate.name ?? ""}` : currentService ? `${t.canvasCaptionInstance}${currentService.name ?? ""}` : currentFlow ? `${t.canvasCaptionInstance}${currentFlow.name ?? ""}` : "";
