	...
	factory: (require) => {
		...
		*/
		function mainViewSessionOf(snapshot) {
			...
			const order = Array.isArray(snapshot.ids) ? snapshot.ids : Object.keys(snapshot.byId);
...
.wf-combo-card{display:flex;gap:9px;align-items:flex-start;text-align:left;border:1px solid var(--wf-border);border-radius:11px;background:var(--wf-layer-2);padding:10px 11px;cursor:pointer;transition:border-color .14s ease,background .14s ease;min-height:96px}
...
`;
		...
		*/
		...
		//#endregion
		//#region \0dsh-global-css:/Users/3phone/workspace/dsh-Visual-Workflow/src/client/entry.css.mjs
		const css = "";
