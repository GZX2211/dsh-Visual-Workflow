...
export class CorruptJsonError extends Error {
    ...
    constructor(filePath, cause) {
        super(`损坏的 JSON 文件：${filePath}（解析失败：${cause?.message ?? String(cause)}）`);
 ...
 * 必须抛出可诊断错误」的契约。
 */
function isTransientLockOpenFailure(error) {
    const code = error?.code;
    return code === 'EPERM' || code === 'EACCES';
}
/** 判断 Windows 报告「文件不存在」的变体 code（跨平台路径存在的鲁棒处理）。 */
function isAbsent(error) {
    return isENOENT(error) || error?.code === 'ENOTDIR';
}
...
export async function withFileLock(path, fn) {
    ...
    try {
        ...
        if (inProcessLocks.get(path) === tail)
 ...
 */
async function tryCreateLockFile(lockPath, now) {
    ...
    try {
        ...
        if (isTransientLockOpenFailure(error)) {
            return { transient: true, code: String(error.code ?? '') };
        }
        throw error;
    }
    try {
 ...
 */
async function tryReapStaleLock(lockPath, staleAfterMs, now, forceWindowsTransition = false) {
    let info;
    try {
    ...
    if (!forceWindowsTransition && now() - mtimeMs < staleAfterMs)
        return false; // 未超阈值：新鲜锁，不动。
    ...
    if (pid !== undefined && isProcessAlive(pid))
        return false; // 持有者还活着：活跃锁，不动。
    // 删除前重新 stat 确认仍是同一锁文件（防误删刚被新持有者替换的锁）。
    const after = await stat(lockPath).catch(() => null);
    if (after === null)
        return false; // 已消失，无需回收。
    if (after.mtimeMs !== mtimeMs || after.size !== info.size)
        return false; // 期间被替换，放弃。
 ...
 */
export async function acquireDiskLock(lockPath, opts) {
    ...
    for (;;) {
        ...
        if ('info' in attempt && attempt.info !== null)
            return attempt.info;
        ...
        if (now() - startedAt >= timeoutMs) {
            throw new DiskLockError(`获取磁盘锁超时（${timeoutMs}ms）：${target}（最后失败原因：${lastFailure}）`, target);
        }
 ...
 */
export async function releaseDiskLock(info) {
    try {
 ...
 */
export async function withJsonLock(filePath, fn, opts) {
    const target = resolve(filePath);
 ...
 * @param opts 可注入临时目录。
 */
export async function atomicReplaceFile(filePath, data, opts) {
    const target = resolve(filePath);
...
function tempFileOwnerPid(entry) {
 ...
 * @returns 被清理的文件路径数组。
 */
export async function cleanupStaleTemp(dir, opts) {
    ...
    for (const entry of entries) {
        ...
        if (entry.endsWith('.lock')) {
            ...
            if (await tryReapStaleLock(full, staleAfterMs, now))
                cleaned.push(full);
        }
    }
    return cleaned;
