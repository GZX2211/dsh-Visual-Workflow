...
export class ServiceManager {
    ...
    async startInner(id) {
        ...
        if (!validation.ok) {
            throw new ServiceManagerError(SERVICE_ERR.FLOW_INVALID, `服务工作流校验未通过：${validation.issues[0]?.message ?? '未知问题'}`);
        }
        const port = await (this.deps.findPort ?? ((base) => findFreePort(base)))(this.deps.config.servicePortBase);
        const patchPath = this.patchPath(id);
        const pluginEntryUrl = new URL('../service-runner.js', import.meta.url).href;
        await atomicReplaceFile(patchPath, Buffer.from(renderServePatch({
            serviceId: id,
        ...
        const managed = { child, port, stopping: false, forceKill: null };
        ...
        child.once('exit', (code, signal) => {
            ...
            if (!entry)
                return;
            if (entry.forceKill)
                clearTimeout(entry.forceKill);
            this.forget(id);
            if (entry.stopping)
                return; // 主动停止：文档状态已置 stopped
    ...
    async stop(serviceId) {
        const id = sanitizeServiceId(serviceId);
        if (!(await this.deps.store.getServiceById(id))) {
            throw new ServiceManagerError(SERVICE_ERR.NOT_FOUND, `服务不存在：${id}`);
        }
        ...
        if (managed) {
            ...
            if (managed.forceKill)
                clearTimeout(managed.forceKill);
    ...
    async autoRecover() {
        ...
        for (const service of running) {
            if (service.status !== 'running' || this.children.has(service.id))
                continue;
            try {
    ...
    spawnChild(dshCommand, serviceId, port, patchPath) {
    ...
    async persistRuntime(serviceId, patch) {
        ...
        await this.deps.store.saveService(next, current.sessionId);
    }
}
 ...
 */
export function resolveDshCommand(envPath = process.env.PATH ?? '') {
    ...
    throw new ServiceManagerError(SERVICE_ERR.DSH_NOT_FOUND, '未找到 dsh 命令（PATH 中无 dsh）；请安装 dsh 或将其所在目录加入 PATH 后重试');
}
