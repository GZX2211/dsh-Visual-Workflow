...
import { dotProduct } from './engine.js';
/**
 * 简易中英混合分词（BM25 词袋）：
 ...
 */
export function bm25Search(chunks, queryTokens, topK) {
    ...
    for (let i = 0; i < chunks.length; i += 1) {
        ...
        if (score > 0)
            scored.push({ index: chunks[i].index, text: chunks[i].text, score, rowKey: chunks[i].rowKey });
    }
...
export class VectorIndex {
    ...
    async load() {
        return readJson(this.filePath, null);
    }
    /**
     ...
     */
    async search(query, topK, engine, options) {
