...
function makeDelivery(host) {
    return {
        ...
        notifyParent({ sessionId, message }) {
            ...
            if (root && typeof root.steer === 'function')
                root.steer(message);
        },
    };
}
...
export function registerWfAskAgent(ctx, host) {
    ...
    const def = defineTool({
        name: WF_ASK_AGENT,
        description: 'Exchange blocking messages between agent nodes of the running workflow. ' +
        ...
        parameters: {
            cmd: { type: 'string', required: true, enum: ['ask', 'reply', 'resolve'], description: 'ask: send and block; reply: answer an ask; resolve: settle a timed-out ask (parent only).' },
            ...
            action: { type: 'string', enum: ['continue', 'resend', 'abort'], description: 'resolve action: continue waiting, resend the ask, or abort it (the sender gets a timeout error).' },
        },
        output: {
            schema: {
                type: 'object',
                ...
                properties: {
                    cmd: { type: 'string', required: true, enum: ['ask', 'reply', 'resolve'], description: 'The command that was executed.' },
                    ...
                    action: { type: 'string', enum: ['continue', 'resend', 'abort'], description: 'The resolve action taken.' },
                },
            },
            render: textRender,
        },
...
//# sourceMappingURL=wf-ask-agent.js.map