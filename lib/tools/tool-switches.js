 ...
 */
export const DEFAULT_DISABLED_TOOLS = ORG_AUTHORING_TOOLS;
/** 空文档（磁盘主体恒为「用户关闭项」；默认关闭种子只在内存权威快照里生效）。 */
export function emptyToolSwitchDoc() {
    return { disabled: [] };
}
...
export class ToolSwitchStore {
    ...
    async readDoc() {
        try {
            const doc = await readJson(this.path(), null);
    ...
    async readDisabledWithDefaults() {
        const doc = await this.readDoc();
    ...
    async setDisabled(name, disabled) {
        ...
        let enabledForSnapshot = new Set();
     ...
     * @returns 更新后的完整关闭清单。
     */
    async setDisabledMany(names, disabled) {
 ...
 */
export function filterToolsInAssembly(assembly, disabled) {
    if (disabled.size === 0)
        return assembly;
    ...
    const sections = Array.isArray(assembly.sections)
        ? assembly.sections.filter((section) => {
            ...
            if (isCodeProtocolSection(name))
                return true; // Code Mode 协议段恒保留
            if (name.startsWith('tool:'))
                return !disabled.has(name.slice('tool:'.length)); // 工具散文段按开关
            return true;
        })
        : assembly.sections;
 ...
 *   - 返回 disposer（插件卸载时撤销）。
 */
export function registerToolSwitchFilter(ctx, store) {
...
//# sourceMappingURL=tool-switches.js.map