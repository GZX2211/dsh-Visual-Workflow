 ...
 */
export function sanitizeReadOnlySql(raw) {
    ...
    if (literalFree.includes(';'))
        return { ok: false, error: '仅允许单条语句（禁止多语句）' };
    if (!/^select\b/i.test(single))
        return { ok: false, error: '仅允许只读 SELECT 查询' };
    ...
    if (blocked.test(literalFree))
        return { ok: false, error: '包含被禁止的写/DDL 关键字' };
    if (/\binto\s+(outfile|dumpfile)\b/i.test(literalFree))
        return { ok: false, error: '禁止导出文件' };
    if (!/\blimit\s+\d+/i.test(single))
        return { ok: false, error: 'SELECT 必须携带 LIMIT 行数限制' };
...
export function registerDataTools(ctx, host) {
    ...
    const definition = defineTool({
        name: WF_DB_QUERY,
        description: 'Query a database node of the active Visual Workflow run. Use only while an orchestration is running and your node is connected to the database via a db-in edge: pass the database node id and pick a mode — ' +
