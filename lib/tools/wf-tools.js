...
function combinedSignal(runSignal, callerSignal) {
    if (typeof AbortSignal.any === 'function')
        return AbortSignal.any([runSignal, callerSignal]);
 ...
 */
export function registerWfTools(ctx, host) {
    ...
    const wfAskDef = defineTool({
        name: WF_ASK,
        description: 'Ask the main-session user questions on behalf of a workflow child agent. ' +
        ...
        async execute(args, exec) {
            ...
            if (!parentRoot)
                throw new WfError('主会话 Agent 未激活，无法向用户提问', 'WF_NO_ROOT_AGENT');
...
//# sourceMappingURL=wf-tools.js.map