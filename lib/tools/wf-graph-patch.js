// src/host/tools/wf-graph-patch.ts
//
...
import { GROUP_HINTS, groupsOf, unknownOpsOf, } from './wf-graph-patch-types.js';
...
function assertSingleGroup(ops) {
    ...
    if (groups.length === 0)
        throw new WfError('补丁为空：ops 至少需要一个操作', 'WF_GRAPH_INVALID');
    if (groups.length > 1) {
...
async function runMetaGroup(host, input, ops) {
    const doc = (await loadDoc(host, input));
    const merged = {};
    for (const op of ops)
        Object.assign(merged, op.meta ?? {});
...
async function runMarkGroup(host, sessionId, ops) {
    ...
    for (const op of ops) {
        ...
        if (mainId !== parentId) {
            throw new WfError(`mark_node:「${requested}」不是当前父代理闸门节点（当前闸门：${entry.milestoneProxyId ?? parentId}）`, 'WF_MILESTONE_INVALID');
        }
    ...
    if (host.persistRun)
        await host.persistRun(snapshot.id);
...
export async function executeGraphPatch(host, sessionId, args) {
    ...
    const origin = args?.origin === 'user' ? 'user' : 'agent';
    ...
    if (group === 'graph') {
        const { revision, issues, result } = await runGraphGroup(host, baseInput, ops);
        return {
            ok: true,
            scope,
            targetId,
    ...
    if (group === 'meta') {
        const { revision, issues, meta } = await runMetaGroup(host, baseInput, ops);
        return { ok: true, scope, targetId, revision, applied: ops.length, warnings: warningsOf(issues), meta };
    }
    const { marked, issues } = await runMarkGroup(host, sessionId, ops);
...
export function registerWfGraphPatch(ctx, host) {
    ...
    const def = defineTool({
        name: WF_GRAPH_PATCH,
        description: 'Apply one patch to a workflow template or the running instance. This tool has three op groups: graph structure, meta parameters, and run-state marking. A patch must use ops from ONE group at a time. ' +
        ...
        parameters: {
            ...
            origin: { type: 'string', enum: ['agent', 'user'], description: 'Change origin; default agent. Org-budget guardrails apply only to agent changes.' },
            ...
            ops: {
                type: 'array',
                required: true,
                description: 'Patch operations; all ops must belong to ONE group (graph | meta | mark). '
...
//# sourceMappingURL=wf-graph-patch.js.map