...
export const ORCH_HARD_CONSTRAINTS = {
     ...
     */
    nodeSettledSignal: '节点完成判定：只有收到结算通知（Background subagent … finished …）才算该节点完成',
    ...
    askAgentTimeout: '收到 wf_ask_agent 的 ask 超时通知时，先用 ask_user_question 征询用户，再用 wf_ask_agent resolve（continue / resend / abort）定案',
...
function buildMidSection(facts) {
    ...
    if (nodeList)
        midParts.push('', '待编排节点（启动时快照，不作为调度依据）：', nodeList);
...
function renderDynamicState(dynamic) {
    ...
    if (dynamic.isResume) {
        ...
        lines.push(`- 已 ok 的节点不得重跑；从节点 ${dynamic.resumeFromNodeId ?? '（未指定）'} 开始，使用检查点产出注入 ctx。`);
    }
    else {
        lines.push('- 全新运行；无可恢复的检查点。');
    }
    if (pauseIds) {
        lines.push(`- 暂停节点：[${pauseIds.join(', ')}]。以其中任一 nodeId 调用 wf_run_node 会暂停运行并持久化检查点；之后从其 flow-out 恢复继续。`);
    }
    else {
