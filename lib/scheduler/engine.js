...
import { nextTriggerAt } from './trigger.js';
import { instantiateFromTemplate, overwriteInstanceFromTemplate } from './instantiate.js';
import { emptyPersistedRuntime, SchedulerTaskStore } from './task-store.js';
...
export class SchedulerEngine {
     ...
     *
     * 重复 start 的 disposer 语义（2026.10 固化）：定时器全对象唯一，重复 start 不新建
     * 定时器，但返回的 disposer 仍能停止当前定时器（而不是空操作）——「start 返回的
     * disposer 负责停掉本引擎的扫描定时器」是调用方可依赖的契约；若返回空操作，第二个
     * 调用方拿到的是假 disposer，会误以为已停止而实际在跑。两个 disposer 都停同一计时器
     * 是幂等的（clearInterval + 置 null），后停者无副作用。
     */
    start() {
        if (this.disposed) {
            this.deps.logger?.warn?.('[visual-workflow] 调度引擎已 dispose，忽略 start');
            return () => { };
        }
        if (!this.timer) {
    ...
    async sweep() {
        ...
        const task = this.sweepOnce();
        this.sweeping = task;
        try {
            await task;
        }
        finally {
            if (this.sweeping === task)
                this.sweeping = null;
        }
    }
    /** 扫描执行体（重入去重保护下运行）。 */
    ...
    async runTaskOnce(task) {
        ...
        if (nextT === null || nextT > now)
            return;
        // 触发点已到且窗口有效（nextTriggerAt 已保证点落在窗口内）：
        await this.fire(task, rt, nextT);
    }
    /** 触发执行：模板 → 实例（会话策略分派 + 每会话单实例覆盖）→ startRun。 */
    async fire(task, rt, triggerAt) {
        rt.lastConsumedTriggerAt = triggerAt;
        try {
            // 新会话工作区：cwd 决策归 sessions 模块的唯一实现（显式 workspacePath 优先，
            // 否则继承创建者会话 cwd——官方会话 header.cwd = 沙箱 workspace-write 根）。
            // 显式路径的存在性校验属保存端点（见 ../workspace-path.ts），运行期不重复读盘。
            const cwd = await resolveNewSessionCwd({
                workspacePath: task.workspacePath,
                creatorSessionId: task.ownerSessionId,
                sessionCwdOf: this.deps.sessionCwdOf,
            });
            ...
            if (!template)
                throw new Error(`工作流模板不存在：${task.workflowTemplateId}`);
    ...
    async suspendByWindow(task, rt, now) {
        if (!rt.currentRunId)
            return;
        try {
    ...
    async resumeWindowPaused(task, rt, now) {
    ...
    async consumeDueTrigger(task, rt, now, result) {
        const nextT = nextTriggerAt(task, rt.lastConsumedTriggerAt ?? this.startedAtMs, task.timezone);
        if (nextT === null || nextT > now)
            return;
...
function deriveStatus(rt) {
    if (!rt.currentRunId)
        return rt.lastResult === 'failed' ? 'error' : 'idle';
    if (rt.windowPausedRunId === rt.currentRunId)
        return 'waiting';
    return 'paused';
}
