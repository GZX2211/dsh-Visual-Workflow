...
import { addDays, dateOnlyOf, formatDateOnly, localToUtc, MAX_SCAN_DAYS, MINUTES_PER_DAY, parseDateOnly, parseTime, zonedParts, } from './calendar.js';
...
export function triggerPointsForDate(task, dateOnly) {
    if (task.triggerMode === 'daily_time') {
        const points = (task.dailyTimeConfig?.timePoints ?? []).map(parseTime).filter((v) => v !== null);
    ...
    if (startFrom === null || !Number.isFinite(interval) || interval < DAILY_TIME_MIN_INTERVAL)
        return [];
    const points = [];
 ...
 */
export function nextTriggerAt(task, afterUtcMs, timeZone) {
