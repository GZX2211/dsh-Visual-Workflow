 ...
 */
export function instantiateFromTemplate(template, sessionId, existingNames, options = {}) {
 ...
 * 不落盘（由调用方 flowStore.saveWorkflow 持久化）。
 */
export function overwriteInstanceFromTemplate(template, existing, options = {}) {
    const now = options.now?.() ?? Date.now();
    return {
        id: existing.id,
