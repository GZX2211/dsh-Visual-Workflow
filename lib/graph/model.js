...
export function nodeHasFlowIn(flow, nodeId) {
    const lines = flow.lines ?? [];
    const hasIn = (id) => lines.some((l) => isFlowLine(l) && l.target === id);
    if (hasIn(nodeId))
        return true;
 ...
 */
export function proxyRoleOf(node) {
    const role = node?.data?.role;
    return role === 'milestone' ? 'milestone' : 'executor';
}
/** 某主节点的**闸门**虚拟节点（role='milestone'）。 */
export function milestoneProxiesOf(flow, nodeId) {
 ...
 */
export function mainNodeIdOf(flow, nodeId) {
