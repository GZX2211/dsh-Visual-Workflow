...
//
// 工作流结构校验（T-013）：单连线问题检测（connectionProblem）与全量校验（validateFlow）。
// 运行前完整性检查（缺失启动/结束节点）归编排运行入口，见文件尾部说明。
//
...
//
// 为什么校验不做「保存前归一化」（2026.10 治理决策）：
// 本模块曾同时提供 validateFlow（只读校验）与 normalizeFlow（保存前补默认值的深拷贝）。
// 后者在生产代码中**从未被调用**，实际落盘形状由各写入路径自己保证：
//   - 画布新建：graph/model.ts 的工厂函数（newRoleNode 等）产出完整 data；
//   - 父代理改图：tools/wf-graph-patch/apply.ts 的 normalizeRoleNodeData（图结构补丁的
//     唯一规范化入口，补缺字段且保留未知字段）；
//   - 导入/模板实例化：模板与实例同源，形状一致。
// 而 normalizeFlow 实现为「顶层字段白名单 + 节点 data 白名单重写」：一旦接入保存端点，
// 会丢掉文档层 meta（自主编排元参数）与 lastPatch（AI 调整角标），并静默丢弃未来新增的
// 节点字段——即它的收益已被别处覆盖、代价却是数据丢失。故删除，避免留下一份「看起来
// 在生效、实际未接入，接入即致损」的伪契约。
import { NODE_HANDLES, HANDLE_PAIRING, CONDITION_TYPES, NODE_KINDS, stageLabel } from './model.js';
/**
 * 检测单条连线的连接点合法性（矩阵 + 通道配对 + 端点存在 + 自环）。
 * 纯函数：不改写任何输入。
 */
export function connectionProblem(nodes, line) {
...
export function validateFlow(flow) {
    ...
    const mode = flow.mode === 'mode2' ? 'mode2' : 'mode1';
    // —— 节点合法性 ——
    const seen = new Set();
    ...
    for (const node of nodes) {
        ...
        if (!NODE_KINDS.includes(node.kind)) {
            issues.push({ code: 'badNode', message: `节点 ${node.id} 的 kind 非法：${String(node.kind)}` });
            continue;
        }
        if (seen.has(node.id))
            issues.push({ code: 'dupNode', message: `节点 id 重复：${node.id}`, id: node.id });
        seen.add(node.id);
    ...
    for (const node of nodes) {
        if (node.kind !== 'proxy' || !node.proxySourceId)
            continue;
    ...
    for (const [key, arr] of inByTarget) {
        ...
        for (const l of arr) {
            ...
            const effectiveIds = new Set([sourceId]);
            if (source.kind === 'proxy' && source.proxySourceId)
                effectiveIds.add(source.proxySourceId);
            if (source.kind === 'parent' || source.kind === 'agent') {
                // 主节点：收集其虚拟节点 id
                for (const p of nodes) {
                    if (p.kind === 'proxy' && p.proxySourceId === sourceId)
    ...
    for (const line of lines) {
        ...
        if (line.condition) {
            if (!CONDITION_TYPES.includes(line.condition.type)) {
                issues.push({ code: 'badCondition', message: `非法条件类型：${String(line.condition.type)}`, id: line.id });
            }
    ...
    if (starts.length > 1)
        issues.push({ code: 'startUnique', message: `启动/输入节点只能存在一个（当前 ${starts.length} 个）` });
    if (ends.length > 1)
        issues.push({ code: 'endUnique', message: `结束/输出节点只能存在一个（当前 ${ends.length} 个）` });
    ...
    if (parents.length > 1)
        issues.push({ code: 'parentUnique', message: `父代理节点最多一个（当前 ${parents.length} 个）` });
...
// 运行前完整性检查（缺失的启动/输入与结束/输出节点）**不在本模块**：
// 它属于编排运行入口的职责，唯一实现在 orchestrator/graph-facts.ts 的
// `missingStageLabels`（按模式渲染中文标签并组装 WfError）。
// 本模块曾另有等价的 missingStageNodes（返回 'start'/'end' 英文键），生产代码零调用，
// 2026.10 治理中删除——同一判定只保留一处实现，避免口径各自漂移。
