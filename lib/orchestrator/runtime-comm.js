...
import { ASK_MESSAGE_LIMIT, buildAskText, buildTimeoutText, coordinatorMessage, } from './ask-protocol.js';
import { statusText, truncateText } from './snapshot.js';
import { WfError, messageOf } from './errors.js';
import {} from './seams.js';
...
export class RuntimeComm extends RuntimeExecute {
     ...
     */
    async targetUnknownHint(run, from, metaFrom, rawTo) {
        ...
        const group = nodes.find((n) => n.kind === 'group' && (n.data.memberIds ?? []).includes(metaFrom.nodeId));
        const memberIds = (group?.data?.memberIds ?? []);
        if (isFlowNode) {
            const labels = memberIds.map((id) => `「${id}」`).join('、');
            return `目标 ${rawTo} 是该工作流的节点，但尚未被启动或不属于当前运行；该成员可能还未被父代理调度，请稍后重试或请父代理先行调度。${labels ? `你可向组内成员发起：${labels}` : ''}`;
        }
     ...
     * WF_CANCELLED 释放。
     */
    async wfAskAgent(caller, childId, args, delivery, callerSignal) {
        ...
        if (cmd === 'ask') {
            ...
            if (metaTo.retired === true) {
                throw new WfError(`目标子代理已被更新配置替换（节点 ${metaTo.nodeId}）；请用节点 id「${metaTo.nodeId}」重新寻址当前子代理`, 'WF_ASK_TARGET_UNKNOWN');
            }
            if (to === from)
                throw new WfError('不能向自己发起协作通信', 'WF_BAD_ARGS');
            const message = String(args?.message ?? '').trim();
            if (!message)
                throw new WfError('wf_ask_agent ask 需要 message', 'WF_BAD_ARGS');
        ...
        if (cmd === 'reply') {
            ...
            if (!askId)
                throw new WfError('wf_ask_agent reply 需要 askId', 'WF_BAD_ARGS');
            const pending = run.asks.get(askId);
            if (!pending)
                throw new WfError('协作通信不存在或已结束', 'WF_ASK_NOT_FOUND');
            if (pending.to !== childId)
                throw new WfError('只有消息目标可以回复该协作通信', 'WF_ASK_MISMATCH');
            if (pending.state !== 'pending')
                throw new WfError('该协作通信已超时，等待父代理裁决', 'WF_ASK_NOT_PENDING');
            ...
            const message = String(args?.message ?? '').trim();
            if (!message)
                throw new WfError('wf_ask_agent reply 需要 message', 'WF_BAD_ARGS');
            if (pending.timer)
                clearTimeout(pending.timer);
            pending.timer = null;
        ...
        const askId = String(args?.askId ?? '').trim();
        if (!askId)
            throw new WfError('wf_ask_agent resolve 需要 askId', 'WF_BAD_ARGS');
        const pending = run.asks.get(askId);
        if (!pending)
            throw new WfError('协作通信不存在或已结束', 'WF_ASK_NOT_FOUND');
...
//# sourceMappingURL=runtime-comm.js.map