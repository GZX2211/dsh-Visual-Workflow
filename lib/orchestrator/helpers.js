...
import { buildParentExecutorPrompt, buildParentTaskSpec } from '../prompts/executor.js';
 ...
 * 非组内成员返回空串（不注入）。
 */
export function collabBlockOf(flow, nodeId) {
    const group = flow.nodes.find((n) => n.kind === 'group' && (n.data.memberIds ?? []).includes(nodeId));
    if (!group)
        return '';
...
export function buildNodeContextFacts(input) {
    ...
    for (const edge of ctxInEdges(flow, node.id)) {
        ...
        if (!entry || (entry.status !== 'ok' && entry.status !== 'react-capped'))
            continue;
        const output = String(entry.output ?? '').trim();
        if (!output)
            continue;
