...
export class RuntimeObserve extends RuntimeComm {
    ...
    deferSubagentEnd(info, tries) {
        ...
        setTimeout(() => {
            ...
            if (this.childIndex.has(childId))
                void this.handleSubagentEnd(info);
            else
    ...
    async markGroupOkIfComplete(entry, memberNodeId) {
        ...
        for (const group of flow.nodes) {
            ...
            const groupHasPendingAsk = Array.from(entry.asks.values()).some((ask) => (ask.state === 'pending' || ask.state === 'timed-out') && (ask.toNodeId === group.id || memberSet.has(ask.toNodeId)));
