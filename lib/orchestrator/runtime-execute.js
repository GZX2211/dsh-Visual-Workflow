...
import { createWaiter } from './run-types.js';
import { setNodeStatus, statusText, terminalizeNodes } from './snapshot.js';
import { GLOBAL_RUN_CALL_LIMIT, WfError } from './seams.js';
...
export class RuntimeExecute extends RuntimeLaunch {
    ...
    requireActiveRootRun(caller, toolName) {
        if (caller.isChild)
    ...
    async wfRunNode(caller, args, callerSignal, options = {}) {
        ...
        if (this.deps.dbIndexer) {
            await ensureDatabaseIndexes(this.deps.dbIndexer.dataDir, resolvedNodeId, flow, this.deps.dbIndexer.engine, this.log());
        }
