...
import { CordisToolsView, NodeAgentRunner, childVisibilityContribution, } from './agent/runner.js';
...
import { registerWfOrgCatalog } from './tools/wf-org-catalog.js';
import { registerWfGraphPatch } from './tools/wf-graph-patch.js';
...
export class VisualWorkflowHost extends Service {
    ...
    constructor(ctx, config, options = {}) {
        ...
        this.orchestrator = new OrchestratorRuntime({
            ...
            systemLanguage: () => systemLanguageOf(ctx.get('settings')),
            logger: cordisLogger(ctx),
        });
    ...
    onAgentSessionStart(payload) {
        ...
        if (!state)
            return; // 非视觉工作流子代理（既不处于首建 pending，也不是已知视觉工作流子代理）
    ...
    async [Service.init]() {
        ...
        try {
            this.ctx.effect(() => registerArrangeCommand(this.ctx, { systemLanguage: () => systemLanguageOf(this.ctx.get('settings')) }), 'visualWorkflowHost.arrangeCommand');
        }
        catch (error) {
    ...
    ecosystemAdapters() {
        ...
        return {
            ...
            listTools: async () => {
                // 与 createSession/presets 端点同款来源（官方 tools 服务的全局 schema 清单）
                ...
                try {
                    ...
                    return (Array.isArray(schemas) ? schemas : []).map((schema) => {
                        const entry = schema;
                        return { name: String(entry.name ?? ''), description: String(entry.description ?? '') };
            ...
            listModels: async () => {
                ...
                for (const entry of llm.listProviders() ?? []) {
                    const provider = typeof entry === 'string' ? entry : String(entry?.id ?? '');
                    if (!provider)
                        continue;
                    try {
            ...
            activeRunOf: (sessionId) => this.orchestrator.activeRunForSession(sessionId),
            currentResolvedFlowOf: async (sessionId) => {
                const entry = this.orchestrator.activeRunForSession(sessionId);
                if (!entry)
                    return null;
                return await this.orchestrator.currentResolvedFlow(entry);
            },
            touchRun: (sessionId) => { this.orchestrator.touchRunForSession(sessionId); },
