...
> **版本适配**：本插件适配 **DeepSeek Harness `0.1.7-rc.1`**（宿主侧 Agent 创建事件、prompt 段名、子代理投递 API、会话 cwd / 当前会话读取、预设 standing scope 与设置读取均按该版本取证适配）。请先安装或升级宿主（升级前先在外部终端**完全停止 dsh 进程**）：
>
> ```bash
...
1. **打开工作台** – 点击官方**左侧边栏底部的「工作流」入口** → 官方**右侧 Sidebar** 打开「工作流」标签页（工作台 = 该标签页；关闭标签页只是隐藏，重开状态原样保留）
2. **新建工作流模板** – 左侧「工作流」→ 下方模板区点击 `＋` 创建空白模板
3. **创建角色模板** – 左侧「角色」→ `＋` → 配置 prompt / 模型 / 工具组合 → 保存
4. **拖拽至画布** – 从左侧拖入角色/文件/数据库/阶段/协作组等模板，生成节点
5. **连线** – 从右侧输出点拖到左侧输入点（`flow` 控制顺序，`ctx` 传递上下文，`db` 注入数据工具）
6. **创建实例并运行** – 点击画布上方「创建实例」（或直接点击「运行」，自动创建实例），模式一启动流程；模式二点击「运行」启动 API 服务
7. **实时编辑** – 运行中可修改画布并保存，后续调度即时生效；节点状态高亮回显，支持撤销/重做
8. **断点续跑** – 暂停节点或关闭窗口后，再次运行从断点继续；历史面板可查看所有 run 记录并恢复中断的流程

> **工作台入口**：官方左侧边栏底部「工作流」按钮 → 官方右侧 Sidebar「工作流」标签页（点击为幂等聚焦，不切换收起）；运行时会自动把全屏的右侧 Sidebar 缩回普通态，给画布让出空间。
...
> 开发/验证目标宿主版本：**`@deepseek-ai/dsh@0.1.7-rc.1`**（`npm install -g @deepseek-ai/dsh@0.1.7-rc.1`；升级前先完全停止 dsh 进程）。
