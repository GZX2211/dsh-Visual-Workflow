...
// （用户裁决）：
//   - instanceOptions 是「从模板创建实例」的一次性临时选项：创建实例时**消费**
//     （读取 newSession/workspacePath 决定目标会话），**不持久化到模板/实例文档**
//     （需求 §4.5.3）；代码中唯一的重置发生在 OPEN_FLOW_TEMPLATE（打开模板时回到
//     「不新开会话」默认值），实例创建路径不回写该状态。
