// src/client/components/panels/inspector/field-forms.tsx
//
// 属性面板字段表单集合（照搬旧项目 node-panels.js，TSX 化 + 按需求 §4.2.2~§4.3 适配）：
// 字段原语（Field/InputField/TextAreaField/NameField）与字段归一化（name/label 取值口径），
// 以及按编辑器对象分发的表单——模板（name 数据源）/ 画布节点（label 数据源）共用；
// 阶段/虚拟节点只读；协作组含成员管理；连线含类型（流程/通过/不通过/内容）与内容值。
