...
export const zh = {
  ...
  // 自动布局重叠提示（自主编排方案 §7.3：非阻断，尊重用户当前布局）
  ...
  // 草稿默认名（本地草稿未命名时使用；与模板默认名分开——实例/模板语义不同）
  ...
  toastStopped: '运行已停止',
  toastSaved: '已保存',
  // 乐观锁冲突（后端稳定码 ERR_REVISION_CONFLICT）：刷新列表后提示用户重试保存
  ...
  // 星期标签（周日 → 周六；索引与 ScheduledTask.window.daysOfWeek 的 0..6 对齐）
  schedulerWeekdays: ['日', '一', '二', '三', '四', '五', '六'],
...
export const en: Dict = {
  ...
  // Canvas caption prefixes (template / instance)
  ...
  // Draft default names (used when a local draft has no name; separate from template defaults)
  ...
  toastSaved: 'Saved',
  // Optimistic-lock conflict (backend code ERR_REVISION_CONFLICT): reload lists, ask user to retry
  ...
  // Weekday labels (Sunday → Saturday; index aligns with daysOfWeek 0..6)
  schedulerWeekdays: ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'],
