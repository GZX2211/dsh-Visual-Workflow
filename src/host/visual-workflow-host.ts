...
import { registerWfOrgCatalog, type OrgCatalogHost } from './tools/wf-org-catalog.js'
import { registerWfGraphPatch, type GraphPatchHost } from './tools/wf-graph-patch.js'
...
export class VisualWorkflowHost extends Service {
   ...
   */
  private ecosystemAdapters(): OrgCatalogHost & GraphPatchHost {
    const ctx = this.ctx
    return {
      store: this.store,
      ...
      listTools: async () => {
        ...
        const tools = ctx.get('tools') as { schemas?: () => unknown } | null | undefined
   ...
   */
  onAgentStatus(payload: { agent?: { id?: unknown }; status?: unknown }): void {
    if (this._disposed) return
    if (String(payload?.status ?? '') !== 'running') return
    const sessionId = String(payload?.agent?.id ?? '')
    if (!sessionId) return
