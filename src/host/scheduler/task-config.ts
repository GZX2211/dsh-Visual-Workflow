 ...
 */
export function parseScheduledTaskInput(raw: unknown, options: { now?: () => number } = {}): ScheduledTask {
  if (!raw || typeof raw !== 'object') throw new ScheduledTaskInputError('requires task')
  const input = raw as Partial<ScheduledTask>
  const taskId = String(input.taskId ?? '')
  if (!taskId || !taskId.startsWith('task-')) throw new ScheduledTaskInputError('定时任务 id 必须以 task- 前缀')
...
// 常用时区建议列表已提升为共享协议常量（唯一本体）：
// 见 src/host/shared/protocol.ts 的 SCHEDULER_TIMEZONE_SUGGESTIONS（host/client 共用，
// 消除此前 host 侧 COMMON_TIMEZONES 与 client 侧 TIMEZONE_SUGGESTIONS 的双份维护）。
// 本模块不再自持该清单——它属于「展示建议」，不参与任务校验（时区合法性由 IANA 解析裁决）。
