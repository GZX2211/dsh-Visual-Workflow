...
export const ORG_PLAN_HARD_CONSTRAINTS = {
  ...
  /** P1 决策：两个编排工具默认关闭。 */
  toolsMayBeClosed: "wf_org_catalog 与 wf_graph_patch 默认关闭；工具不可用时提示用户开启后重试",
} as const
...
function describeTarget(facts: OrgPlanPromptParams['facts']): { identity: string; goal: string; grammar: string } {
 ...
 */
export function buildOrgPlanPrompt(params: OrgPlanPromptParams): string {
...
function renderPlanDynamicState(dynamic: OrgPlanPromptParams['dynamic']): string {
  const lines: string[] = ['当前规划任务：']
