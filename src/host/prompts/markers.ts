...
export const HEAD_MARKER = '## HARD CONSTRAINTS (read first — non-negotiable)'

// 中段标记：过程性信息段（节点清单 / 上游产出 / 文件路径索引 / 数据库工具说明）的起始锚。
// 关键约束「双位」之间的中部，只放过程性信息，不放硬约束（lost-in-the-middle 处置）。
export const MID_MARKER = '## PROCESS CONTEXT (procedural reference only)'

// 尾段标记：动态段（断点继续 / 暂停 / 运行参数 / 本次状态等不稳定内容）的起始锚。
// 承诺：构建器输出中，从首个「TAIL_MARKER 行」开始直到末尾为动态段，
//       其之前（含首段硬约束 + 中段过程性信息）为同一 run 内字节稳定的前缀。
export const TAIL_MARKER = '## DYNAMIC STATE (injected only at tail — do not reorder)'

// 尾段重申标识：末段对首段关键约束的重申小节标题。
// 保证测试可断言「关键约束短语同时出现在首段与末段」的明确位置。
export const TAIL_RESTATE_MARKER = '## Reaffirmation of hard constraints'
