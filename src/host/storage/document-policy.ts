...
export function stripClientMeta<T>(value: T, keepServerFields = false): T {
...
export function keepServerFieldsOf(options: SaveOptions | undefined): boolean {
  return options?.keepServerFields === true
}

/** 提取当前 revision（非法/缺失按 0 处理，旧项目 flowRevision 语义）。 */
export function flowRevision(value: { revision?: number } | null): number {
 ...
 * 本项目把该语义收敛为显式参数）。
 */
export function nextFlowRevision(
  incoming: { revision?: number; id?: string },
