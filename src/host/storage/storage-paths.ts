...
export const NESTED_DIRS = ['data/files'] as const

/** 受管文件目录（data/files，存放非文本文件模板的受管拷贝）。 */
export function managedFilesDir(root: string): string {
  return join(root, 'data', 'files')
}
...
/** 工作流实例文件路径。 */
export function workflowPath(root: string, flowId: string): string {
  return join(root, 'workflows', `${safeFilePart(flowId)}.json`)
}

/** 服务实例文件路径。 */
export function servicePath(root: string, serviceId: string): string {
  return join(root, 'services', `${safeFilePart(serviceId)}.json`)
}

/** 服务会话映射文件路径（services/<id>.sessions.json）。 */
export function sessionsPath(root: string, serviceId: string): string {
  return join(root, 'services', `${safeFilePart(serviceId)}.sessions.json`)
}

/** 模板种类 → 目录（data/ 内 file 与 database 同目录，以字段判别）。 */
export function templateDir(kind: TemplateKind): string {
  return kind === 'role' ? 'roles' : kind === 'group' ? 'groups' : 'data'
}

/** 模板文件路径。 */
export function templatePath(root: string, kind: TemplateKind, id: string): string {
...
/** run 快照文件路径。 */
export function runsPath(root: string, runId: string): string {
  return join(root, 'runs', `${safeFilePart(runId)}.json`)
}

/** 运行时流程定义文件路径（orchestrations/）。 */
export function orchestrationPath(root: string, runId: string): string {
  return join(root, 'orchestrations', `${safeFilePart(runId)}.json`)
}

/** 工具组合文件路径（combos.json 单文件）。 */
export function combosPath(root: string): string {
