...
/** 节点 id 是否为可执行单元。 */
function isUnit(node: GraphNode | undefined): boolean {
...
export function ruleStartRequired({ flow }: CheckGraphInput): GraphIssue[] {
...
export function ruleStartFlowOut(input: CheckGraphInput, dag: FlowDag): GraphIssue[] {
  const { flow } = input
...
/** c) 恰好 1 个结束/输出节点。 */
export function ruleEndRequired({ flow }: CheckGraphInput): GraphIssue[] {
...
export function ruleEndFlowIn(input: CheckGraphInput, dag: FlowDag): GraphIssue[] {
  const { flow } = input
  const byId = nodeMapOf(flow)
  const issues: GraphIssue[] = []
  for (const node of flow?.nodes ?? []) {
...
export function ruleOrphanNode({ flow }: CheckGraphInput): GraphIssue[] {
...
export function ruleConditionPair({ flow }: CheckGraphInput): GraphIssue[] {
  const groups = new Map<string, { pass: string[]; fail: string[]; ids: string[] }>()
  for (const line of flow?.lines ?? []) {
    const type = line.condition?.type
    if (!type || type === 'content') continue
    if (line.sourceHandle !== 'flow-out') continue
    const key = `${line.source}|${line.sourceHandle}`
    const group = groups.get(key) ?? { pass: [], fail: [], ids: [] }
    if (type === 'pass') group.pass.push(line.target)
    else group.fail.push(line.target)
...
export function ruleFlowCycle(_input: CheckGraphInput, dag: FlowDag, cycleNodes: Set<string>): GraphIssue[] {
...
export function ruleReachability(input: CheckGraphInput, dag: FlowDag): GraphIssue[] {
  ...
  for (const node of input.flow?.nodes ?? []) {
    ...
    if (node.kind === 'start' || !hasIncoming) continue
...
export function ruleCannotReachEnd(input: CheckGraphInput, dag: FlowDag): GraphIssue[] {
  const issues: GraphIssue[] = []
  for (const node of input.flow?.nodes ?? []) {
    if (node.kind === 'end') continue
    const hasIncoming = (dag.incoming.get(node.id) ?? []).length > 0
...
export function ruleStageDirection({ flow }: CheckGraphInput, dag: FlowDag): GraphIssue[] {
  const issues: GraphIssue[] = []
  for (const node of flow?.nodes ?? []) {
    if (node.kind === 'pause') {
      const hasIncoming = (dag.incoming.get(node.id) ?? []).length > 0
