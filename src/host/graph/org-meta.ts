...
function positiveInt(value: unknown, cap: number): number | undefined {
 ...
 */
export function normalizeOrgMeta(input: unknown): OrgMeta {
  ...
  if (nodeMin !== undefined && nodeMax !== undefined && nodeMin > nodeMax) {
    ...
    if (nodeMin !== undefined) out.nodeMin = nodeMin
    if (nodeMax !== undefined) out.nodeMax = nodeMax
  }

  const groupMax = positiveInt(raw.groupMax, caps.groupMax)
  if (groupMax !== undefined) out.groupMax = groupMax
  ...
  if (membersMin !== undefined && membersMax !== undefined && membersMin > membersMax) {
    ...
    if (membersMin !== undefined) out.membersMin = membersMin
    if (membersMax !== undefined) out.membersMax = membersMax
  }

  const parallelBranchMax = positiveInt(raw.parallelBranchMax, caps.parallelBranchMax)
  if (parallelBranchMax !== undefined) out.parallelBranchMax = parallelBranchMax
  const milestoneMax = positiveInt(raw.milestoneMax, caps.milestoneMax)
  if (milestoneMax !== undefined) out.milestoneMax = milestoneMax
  const patchOpsMax = positiveInt(raw.patchOpsMax, caps.patchOpsMax)
  if (patchOpsMax !== undefined) out.patchOpsMax = patchOpsMax
  const askPerNodeMax = positiveInt(raw.askPerNodeMax, caps.askPerNodeMax)
  if (askPerNodeMax !== undefined) out.askPerNodeMax = askPerNodeMax

  // TODO(可视化可调项)：以下枚举/布尔语义字段目前只做归一化与持久化，**尚未接入任何行为**——
  // 它们与「组织规划提示词可调项」一起规划为可视化 UI 中的可调旋钮（用户裁决 2026.09：
  // 暂不接入提示词或护栏，先保留字段与归一化，避免契约反复迁移）。接入时的落点：
  //   ① 注入提示词 → org-budget.ts 的 buildOrgBudgetText（父代理据此规划）；
  //   ② 可机械判定的进护栏 → metaLimitIssues / invariants-rules-nodes.ts。
  const planFreedom = oneOf(raw.planFreedom, ['templates-only', 'allow-new-role'] as const)
  if (planFreedom) out.planFreedom = planFreedom
  const promptSource = oneOf(raw.promptSource, ['user-template', 'agent-generated'] as const)
  if (promptSource) out.promptSource = promptSource
  const roleGranularity = oneOf(raw.roleGranularity, ['broad', 'narrow'] as const)
  if (roleGranularity) out.roleGranularity = roleGranularity
  const roleReuse = oneOf(raw.roleReuse, ['forbid', 'allow'] as const)
  if (roleReuse) out.roleReuse = roleReuse
  const crossGroupPolicy = oneOf(raw.crossGroupPolicy, ['via-parent', 'forbid'] as const)
  if (crossGroupPolicy) out.crossGroupPolicy = crossGroupPolicy

  if (Array.isArray(raw.interveneTrigger)) {
    const triggers = raw.interveneTrigger
      .map((item) => oneOf(item, ['user', 'threshold', 'milestone'] as const))
      .filter((item): item is 'user' | 'threshold' | 'milestone' => item !== undefined)
    // 去重后写入（顺序保持首次出现，便于提示词稳定输出）
    if (triggers.length > 0) out.interveneTrigger = [...new Set(triggers)]
  }

  const failure = raw.failurePolicy as Record<string, unknown> | undefined
  ...
  if (Array.isArray(raw.forbiddenShapes)) {
    const shapes = raw.forbiddenShapes.map((item) => nonEmptyString(item)).filter((item): item is string => !!item)
    if (shapes.length > 0) out.forbiddenShapes = [...new Set(shapes)]
  }
  ...
  if (raw.eval && typeof raw.eval === 'object' && !Array.isArray(raw.eval)) {
    out.eval = { ...(raw.eval as Record<string, unknown>) }
  }
  if (raw.restructure && typeof raw.restructure === 'object' && !Array.isArray(raw.restructure)) {
    out.restructure = { ...(raw.restructure as Record<string, unknown>) }
  }
  return out
}
 ...
 */
export function effectiveOrgMeta(...sources: Array<OrgMeta | null | undefined>): OrgMeta {
  const merged: OrgMeta = {}
 ...
 */
export function metaOfDocument(doc: Pick<WorkflowDocument | WorkflowTemplate, 'meta'> | null | undefined): OrgMeta {
 ...
 */
export function orgBudgetOf(meta: OrgMeta, usage: OrgUsage): OrgBudget {
 ...
 */
export function freezeOrgMeta(snapshot: RunSnapshot, meta: OrgMeta | null | undefined): void {
