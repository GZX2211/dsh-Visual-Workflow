...
import { computeFlowLayers, maxLayerWidth, type FlowDag } from './dag.js'
...
export function ruleDuplicateRoleLabel({ flow }: CheckGraphInput): GraphIssue[] {
 ...
 */
export function ruleNamingConvention({ flow, meta }: CheckGraphInput): GraphIssue[] {
  ...
  const issues: GraphIssue[] = []
  for (const node of flow?.nodes ?? []) {
    if (node.kind !== 'agent' && node.kind !== 'parent' && node.kind !== 'group') continue
 ...
 * ——即「同一执行轮次里并行开工的代理数」。
 */
export function ruleMetaLimits(input: CheckGraphInput, dag: FlowDag, cycleNodes: Set<string>): GraphIssue[] {
  if (input.origin !== 'agent' || !input.meta) return []
