...
//   - 任务块组装：buildNodeBlocks（persona 任务 + 上游上下文 + 输入结构 + 交接契约）；
//   - 交接契约：inputContractOf / outputContractOf（ctx-out 出线才注入默认结构）；
...
import { buildNodeTaskBlock, DEFAULT_OUTPUT_CONTRACT } from '../prompts/node-task.js'
 ...
 * 纯函数；未配置返回空串（不组装该段）。
 */
export function inputContractOf(node: RoleNode): string {
 ...
 */
export function outputContractOf(flow: WorkflowDocument, node: RoleNode): { text: string; defaulted: boolean } {
  const configured = String(node.data?.outputSchema ?? '').trim()
  const hasConsumer = (flow.lines ?? []).some(
    (line) => line.source === node.id && line.sourceHandle === 'ctx-out',
  )
  if (configured) {
    return {
