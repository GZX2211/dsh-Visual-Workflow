...
export abstract class RuntimeBase {
   ...
   * 而不必把基类反向依赖到下游继承层。
   */
  abstract resumeRun(input: { sessionId: string; flowId: string; fromRunId?: string }): Promise<ResumeResult>
   ...
   */
  async persistRunSnapshot(entry: RunEntry): Promise<void> {
   ...
   */
  protected async prepareParentExecutor(
    flow: WorkflowDocument,
    entry: RunEntry,
  ): Promise<{ nodeId: string; nodeLabel: string; task: ExecutorContextFacts; runContextText: string } | null> {
   ...
   */
  protected async ensureActiveRun(sessionId: string): Promise<RunEntry | null> {
  ...
  private async adoptRunContext(sessionId: string): Promise<RunEntry | null> {
    ...
    if (!target) {
      let newest: RunSnapshot | null = null
      try {
        for (const runId of await this.deps.store.listAllRunIds()) {
          ...
          if (!(RESUMABLE_STATUSES as readonly string[]).includes(record.status)) continue
          if (!newest || String(record.startedAt ?? '') > String(newest.startedAt ?? '')) newest = record
        }
      } catch (error) {
    ...
    for (const entry of this.runs.values()) {
      const s = entry.snapshot
      if (s.sessionId === sessionId && s.flowId === target && s.status === 'running') return entry
    }
    return null
  }
  ...
  protected autoResumeRun(sessionId: string, flowId: string): Promise<ResumeResult | null> {
    ...
    if (pending) return pending
    const task = (async (): Promise<ResumeResult | null> => {
      try {
        const result = await this.resumeRun({ sessionId, flowId })
        this.log().info(
          `[visual-workflow] 断点自动续跑：flow=${flowId} session=${sessionId} run=${result.runId}（resumedFrom=${result.resumedFromRunId}）`,
        )
        return result
      } finally {
  ...
  touchRunForSession(sessionId: string): boolean {
    ...
    for (const entry of this.runs.values()) {
      const snapshot = entry.snapshot
      if (!snapshot || snapshot.sessionId !== id || snapshot.status !== 'running') continue
      this.touchRun(entry)
      touched = true
    }
    return touched
