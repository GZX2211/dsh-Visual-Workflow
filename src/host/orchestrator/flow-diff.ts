 ...
 */
const GEOMETRY_DATA_KEYS = ['size', 'swapPorts'] as const
...
export function nodeConfigKeyOf(node: GraphNode): string {
  const { position: _position, ...rest } = node as GraphNode & { position?: unknown }
  const data = (rest as { data?: Record<string, unknown> }).data
  if (!data || typeof data !== 'object') return stableStringify(rest)
  const trimmed: Record<string, unknown> = {}
  for (const [key, value] of Object.entries(data)) {
    if ((GEOMETRY_DATA_KEYS as readonly string[]).includes(key)) continue
    trimmed[key] = value
  }
 ...
 */
export function lineKeyOf(line: Line): string {
...
function extraKeys(prev: Map<string, number>, next: Map<string, number>): string[] {
  const out: string[] = []
  for (const [key, count] of next) {
    const before = prev.get(key) ?? 0
    for (let index = before; index < count; index += 1) out.push(key)
  }
  return out
}
 ...
 */
export function summarizeFlowChange(prev: WorkflowDocument, next: WorkflowDocument): FlowChangeSummary {
  ...
  for (const [id, node] of nextNodes) {
    ...
    if (!before) addedNodeIds.push(id)
    else if (nodeConfigKeyOf(before) !== nodeConfigKeyOf(node)) changedNodeIds.push(id)
  }
